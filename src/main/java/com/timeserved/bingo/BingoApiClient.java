...
public class BingoApiClient
{
	...
	/** One open LFG post — mirrors GET /api/lfg's post shape. */
	public static class LfgPost
	{
		public String id;
		public String activity;
		public int spotsNeeded;
		public String note;
		public String partyPassphrase;
		public String createdAt;
		public String postedBy;
		public boolean isMine;
	}

	private static class LfgListResponse
	{
		List<LfgPost> posts;
	}

	/**
	 * Fetches every currently-open LFG post (already age-filtered
	 * server-side — see MAX_AGE_MINUTES in api/lfg.ts).
	 */
	public void fetchLfgPosts(String apiKey, Consumer<List<LfgPost>> onSuccess, Consumer<String> onError)
	{
		HttpUrl url = HttpUrl.parse(BASE_URL + "/api/lfg");
		if (url == null)
		{
			onError.accept("Invalid API base URL");
			return;
		...
		httpClient.newCall(request).enqueue(new Callback()
		{
			...
			public void onFailure(Call call, IOException e)
			{
				log.debug("Failed to fetch LFG posts", e);
				onError.accept("Could not reach the clan site");
			}
			...
			public void onResponse(Call call, Response response)
			{
				try (Response closeable = response)
				{
					...
					LfgListResponse parsed = gson.fromJson(body.charStream(), LfgListResponse.class);
					onSuccess.accept(parsed.posts == null ? Collections.emptyList() : parsed.posts);
				}
				catch (JsonSyntaxException e)
				{
					log.debug("Malformed LFG list response", e);
					onError.accept("The clan site returned an unexpected response");
				}
			}
		});
	...
	/**
	 * Posts (or refreshes) the caller's own LFG ad. The server upserts by
	 * user (see api/lfg.ts), so calling this again with different fields is
	 * how a member edits their existing post rather than stacking a second one.
	 */
	public void postLfg(
		String apiKey, String activity, int spotsNeeded, String note, String partyPassphrase,
		Runnable onSuccess, Consumer<String> onError)
	{
		...
		body.addProperty("spotsNeeded", spotsNeeded);
		body.addProperty("note", note);
		body.addProperty("partyPassphrase", partyPassphrase);
		...
		httpClient.newCall(request).enqueue(new Callback()
		{
			...
			public void onFailure(Call call, IOException e)
			{
				log.debug("Failed to post LFG ad", e);
				onError.accept("Could not reach the clan site");
			}
	...
	/** Cancels the caller's own open LFG post, if any. */
	public void cancelLfg(String apiKey, Runnable onSuccess, Consumer<String> onError)
	{
		HttpUrl url = HttpUrl.parse(BASE_URL + "/api/lfg");
		if (url == null)
		{
		...
		httpClient.newCall(request).enqueue(new Callback()
		{
			...
			public void onFailure(Call call, IOException e)
			{
				log.debug("Failed to cancel LFG post", e);
				onError.accept("Could not reach the clan site");
			}
